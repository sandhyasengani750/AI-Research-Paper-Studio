...
# This is mandatory for Vercel serverless environments.
...
def generate_academic_chart(api_key: str, paper_title: str, results_notes: str, figure_index: int):
    ...
    try:
        ...
        plt.close('all') # Prevent memory leaks in serverless functions
...
def generate():
    ...
    if not api_key:
        # Fallback to form data if not in environment
        api_key = (request.form.get("api_key") or "").strip()
        
    if not api_key:
        return jsonify({"error": "Gemini API Key missing. Enter it in the sidebar or configure GEMINI_API_KEY in environment variables."}), 400
